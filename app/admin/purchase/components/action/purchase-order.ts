// app/admin/purchase/components/action/purchase-order.ts
"use server";
...
export async function fetchPurchaseOrder(poId: string): Promise<ServerResponse<PurchaseOrderDetail>> {
  try {
    console.log("Fetching purchase order detail with ID:", poId);

    const supabase = await createSupabaseAdmin();

    // Step 1: Get basic purchase order info
    ...
    if (poError) {
      console.error("Error fetching purchase:", poError);
    ...
    console.log("PO found:", po.po_number);

    // Step 2: Get vendor info (with image)
    const { data: vendor, error: vendorError } = await supabase
      .from("vendors")
      .select("vendor_id, vendor_name, vendor_image")
      .eq("vendor_id", po.vendor_id)
      .single();

    if (vendorError) {
      console.warn("Warning: Failed to fetch vendor:", vendorError);
    }

    // Step 3: Get purchase items with product images
    ...
    console.log("PO detail loaded successfully");
    return { data: formattedData, error: null };
  } catch (err: any) {
