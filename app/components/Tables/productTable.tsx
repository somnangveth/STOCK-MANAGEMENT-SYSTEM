      ...
      <Table className="w-full border border-gray-300 rounded-xl">
        ...
        <TableHeader className="bg-gray-100">
          <TableRow>
            ...
            {/* price*/}
            {columns.includes("base_price") && (
              <TableHead>Base Price ($)</TableHead>
            )}
            {columns.includes("profit_price") && (
              <TableHead>Profit Price ($)</TableHead>
            )}
            {columns.includes("tax") && <TableHead>Tax ($)</TableHead>}
            {columns.includes("shipping") && (
              <TableHead>Shipping ($)</TableHead>
            )}
            {columns.includes("discount_price") && (
              <TableHead>Discount ($)</TableHead>
            )}
            {columns.includes("total_price") && (
              <TableHead>Total Price ($)</TableHead>
            )}

            {/* ✅ ADDED B2B HEADER */}
            {columns.includes("b2b_price") && (
              <TableHead>Total Price</TableHead>
            )}
        ...
        <TableBody className="bg-white">
          {currentProducts.map((products, index) => (
            <TableRow key={products.sku_code || index}>
              {columns.includes("sku-code") && (
                <TableCell>{products.sku_code}</TableCell>
              )}
                ...
                <TableCell>
                  {products.product_image ? (
                    <img
                      ...
                      alt={String(products.product_name)}
              ...
              {columns.includes("category_id") && (
                <TableCell className="text-center">
                  <p className="bg-amber-100 text-amber-700 rounded-lg">
              ...
              {columns.includes("subcategory_id") && (
                <TableCell className="text-center">
                  <p className="bg-purple-100 text-purple-700 rounded-lg">
                    {products.subcategory_name || "—"}
                  </p>
                </TableCell>
              )}
              {columns.includes("base_unit") && (
                <TableCell>{products.base_unit || "—"}</TableCell>
              )}
              {columns.includes("baseprice") && (
                <TableCell>{products.baseprice ?? "—"}</TableCell>
              )}
              {columns.includes("taxes") && (
                <TableCell>{products.taxes ?? "—"}</TableCell>
              )}
              {columns.includes("totalprice") && (
                <TableCell>{products.totalprice ?? "—"}</TableCell>
              )}
              ...
              {columns.includes("quantity_remaining") && (
                <TableCell>{products.quantity_remaining}</TableCell>
              )}

              {columns.includes("base_price") && (
                <TableCell> $ {products.base_price} </TableCell>
              )}

              {columns.includes("profit_price") && (
                <TableCell>$ {products.profit_price}</TableCell>
              )}

              {columns.includes("tax") && (
                <TableCell>$ {products.tax}</TableCell>
              )}
              {columns.includes("shipping") && (
                <TableCell>$ {products.shipping}</TableCell>
              )}
              {columns.includes("discount_price") && (
                <TableCell>$ {products.discount_price}</TableCell>
              )}
              {columns.includes("total_price") && (
                <TableCell>$ {products.total_price}</TableCell>
              )}

              {/* ✅ ADDED B2B CELL */}
              {columns.includes("b2b_price") && (
                <TableCell>$ {products.b2b_price ?? "—"}</TableCell>
              )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-600">
            Showing {startIndex + 1} to {Math.min(endIndex, product.length)} of{" "}
