...
import { deleteSubcategory } from "@/app/functions/admin/stock/category/subcategory";
...
export default function DeleteSubcategoryForm({subcategory}:{subcategory: Subcategories}){
    const [ open , setOpen ] = useState(false);

    async function handleDelete(){
        try{
            const result = await deleteSubcategory(subcategory.subcategory_id);

            if(!result){
                console.error('Failed to delete');
            }
            styledToast.success("Delete subcategory successfully!");
