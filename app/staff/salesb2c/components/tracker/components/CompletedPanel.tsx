...
import { CompletedBtn } from "@/app/components/ui";
...
export default function CompletedReceiptPanel() {
  ...
  // Filter completed sales with customertype "General"
  const saleData = useMemo(() => {
    if (!allSaleData) return [];
    return allSaleData.filter(
      (sale: Sale) => sale.process_status === "completed" && sale.customertype === "General"
    );
  }, [allSaleData]);
